...
int main() {
  ...
  hipEventRecord(events[0], strms[0]);
  kernel_a<<<gridsize, blocksize,0,strms[0]>>>(d_a, N);
  HIP_ERRCHK(hipGetLastError());
  hipEventRecord(events[1], strms[0]);

  hipEventRecord(events[2], strms[1]);
  kernel_b<<<gridsize, blocksize,0,strms[1]>>>(d_b, N);
  HIP_ERRCHK(hipGetLastError());
  hipEventRecord(events[3], strms[1]);

  hipEventRecord(events[4], strms[2]);
  kernel_c<<<gridsize, blocksize,0,strms[2]>>>(d_c, N);
  HIP_ERRCHK(hipGetLastError());
  hipEventRecord(events[5], strms[2]);
  ...
  hipEventElapsedTime(&atime, events[0], events[1]);
  hipEventElapsedTime(&btime, events[2], events[3]);
  hipEventElapsedTime(&ctime, events[4], events[5]);

  printf("Kernel a time: %f \n", atime);
  printf("Kernel b time: %f \n", btime);
  printf("Kernel c time: %f \n", ctime);
