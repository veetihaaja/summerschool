...
1. In this first optimization, try to reduce the amount of atomicadd to global memory by performing the accumulation on the thread 0. `atomicAdd` in shared memory are way faster, so the idea of this algorithm is that each thread does accumulation in his register, then does an `atomicAdd` in shared memory. After all threads have done so, thread 0 will add this partial sum to the global memory.


## Task: use shared memory to further improve it

1. But we can do better with shared memory! What if we use a "tree" algorithm to reduce the partial sum from all threads to thread 0, instead of using `atomicAdd`?
