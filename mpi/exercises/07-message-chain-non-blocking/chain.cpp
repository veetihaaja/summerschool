...
int main(int argc, char *argv[]) {
    ...
    std::vector<MPI_Request> reqs(ntasks-1);

    // TODO: Send messages
    for (int i=0; i<ntasks-1; i++) {
        MPI_Isend(message.data(), numElements, MPI_INT, destination, 0, MPI_COMM_WORLD, &(reqs.at(i)));
    }
    ...
    for (int i=1; i<ntasks; i++) {
        MPI_Irecv(receiveBuffer.data(), numElements, MPI_INT, source, 0, MPI_COMM_WORLD, &(reqs.at(i-1)));
    }

    if (rank != 0) {
        MPI_Wait(&(reqs.at(rank-1)), MPI_STATUS_IGNORE);
    }
